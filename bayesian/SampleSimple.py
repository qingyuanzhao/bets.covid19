...
parser = OptionParser(description="Samples the posterior distribution of the parameters in the model with E | B uniform, and a single growth rate")
...
def log_posterior_fn(logitPi, logitLamw, logitLamv, logr, g):
    ...
    for t in range(L+1):
        a = ( (1-pi)*lamw/(L+1)*tf.cast(tf.equal(B,0.0),dtype=tf.float32)+                 
                ...
                tf.math.exp(r*t)*tf.cast(tf.greater_equal(E, float(t)),dtype=tf.float32)* \
    ...
    for b in range(L+1):
        for t in range(b,L+1):
            p += ((1-pi)*lamw/(L+1)*(b==0) +    
                  pi/L*lamv/L*(b>0)) *          \
                   tf.math.exp(r*t)*(L+1-t)
    p = tf.math.log(p)
    log_post = f - n*p + log_prior_fn(logitPi,logitLamw,logitLamv,logr,g)
    return log_post
...
output_nm = "simple_alpha_%.3f_copy_%i"%(alpha,options.copy)
if options.location:
