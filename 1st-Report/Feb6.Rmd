...
*This report updates and is the final version of the [2nd preliminary analysis](https://htmlpreview.github.io/?https://github.com/qingyuanzhao/2019-nCov-Data/blob/master/Feb1.html).*
...
\end{equation}
where the $OR_s$ represents the proportion of people leaving Wuhan to the selected countries/regions. We will consider three choices of $(OR_s)_{s=1}^N$:
...
3. $OR_s = 0$ for $1 \le s \le N-1$ and $OR_N = N$. This corresponds to fitting a simple exponential growth model for $OI_t$. This assumption is unrealistic but gives us a sensitivity analysis of our results.

We will illustrate our analysis using the first choice, that is, the travelling rate is constant over the entire period. An immediate consequence of this model is that the logarithm of the expectation of $OI_t$ is given by
...
The three choices of $(OR_s)_{s=1}^N$ are represented by the offset term in this code
```{r}
...
Our initial analysis of $\log(\mathbb{E}[OI_t])$ shows that the last few days seem to exhibit a different pattern, so we exclude them from our analysis. Imputed infection times in the early period are also reliable because they are mostly determined by the tail distribution of incubation period. Thus we fit our model below using the period January 1 to January 20
```{r}
...
settings <- expand.grid(offset = c(1,2,3), end = c(20+31, 23+31), incubation = c("2019-nCoV", "SARS"))
posterior <- list()
for (i in 1:nrow(settings)) {
  ...
  if (settings$incubation[i] == "2019-nCoV") {
    posterior[[i]] <- obtain.posterior(symptom_imputed, stan_data,
                                        incubation_alpha = 1.92, incubation_beta = 0.37)
  } else {
    posterior[[i]] <- obtain.posterior(symptom_imputed, stan_data,
                                        incubation_alpha = 6.37^2 / 4.08^2, incubation_beta = 6.37 / 4.08^2)
  }
}
```
...
posterior.to.output <- function(posterior) {
  posterior.to.output <- function(posterior) { # uses my.summary
    summ <- my.summary(posterior)
    summ <- signif(summ, 2)
    paste0(" ", summ[1], " [", summ[2], ", ", summ[3], "] ")
  }
  r_posterior <- unlist(lapply(posterior, function(posterior) posterior$r))
  r_output <- posterior.to.output(r_posterior)
  dd_output <- posterior.to.output(log(2) / r_posterior)
  R0_output <- posterior.to.output(r.to.R(r_posterior))
